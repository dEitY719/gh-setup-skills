...
# The provenance line that trails the footer block: one standalone line after
# `</details>`. Its byte format is owned upstream by `dEitY719/gh-issue-skills`'s
# `lib/ai-metrics-footer.sh` (gh-issue-skills#43) -- this skill is a consumer and
# never redefines it. A post-hoc backfill cannot know which harness/model wrote
# the card, so it emits `none` rather than guessing (issue #16 D-3).
FOOTER_ORIGIN_UNKNOWN='Generated by Harness(none) and LLM(none)'
...
#
# The trailing `(\n\nGenerated by Harness(..) and LLM(..))?` group makes the
# footer block and its provenance line ONE replaceable unit (issue #16 F-1), so
# a --force re-run cannot leave the line orphaned or duplicated. The group is
# optional, so a pre-#16 card still matches unchanged (D-2), and it is anchored
# immediately after `</details>`, so a `Generated by ...` sentence sitting in
# ordinary prose elsewhere in the body is never swallowed (NF-3 / D-4).
# The captured line is re-emitted verbatim: the real harness/model is only
# knowable from the card itself, and overwriting it with `none` would destroy
# provenance this skill cannot recompute. `none` is for the unknown case only
# -- an old card being upgraded in place.
replace_footer() {
    local body="$1" tokens="$2" human="$3" elapsed="$4" new_block
    new_block=$(printf '<details>\n<summary>🤖 AI Metrics · 📊 ~%s tokens · 👤 ~%s h · 🤖 ~%s min</summary>\n\n<!-- ai-metrics -->\n📊 ~%s tokens · 👤 ~%s h · 🤖 ~%s min\n<!-- /ai-metrics -->\n\n</details>' \
        "$tokens" "$human" "$elapsed" "$tokens" "$human" "$elapsed")
    printf '%s' "$body" \
        | NEW="$new_block" ORIGIN="$FOOTER_ORIGIN_UNKNOWN" perl -0777 -pe '
            BEGIN { $n = $ENV{NEW}; $o = "\n\n" . $ENV{ORIGIN} }
            s|(\n+---\n)(?:<details>\n<summary>[^\n]*</summary>\n\n)?<!-- ai-metrics(?::[A-Za-z0-9_-]+)? -->.*?<!-- /ai-metrics(?::[A-Za-z0-9_-]+)? -->(?:\n\n</details>)?(\n\nGenerated by Harness\([^\n]*\) and LLM\([^\n]*\))?|$1 . $n . (defined $2 ? $2 : $o)|se
          '
}
...
self_test() {
    ...
    # --- Harness/LLM provenance line (issue #16) ---------------------------
    local newfmt noorigin prose prose_tagged
    _ok 'append_footer origin line' \
        "$(append_footer "$body" 1500 2 6 | tail -1)" \
        'Generated by Harness(none) and LLM(none)'

    newfmt=$(printf '%s' "$tagged" \
        | sed 's|Harness(none) and LLM(none)|Harness(claude) and LLM(opus)|')
    # --force leaves exactly one origin line, and keeps the values it found --
    # a backfill can never recompute them, so it must not overwrite them.
    _ok 'replace_footer origin once' \
        "$(replace_footer "$newfmt" 9000 4 12 | grep -c 'Generated by Harness(')" '1'
    _ok 'replace_footer origin kept' \
        "$(replace_footer "$newfmt" 9000 4 12 | tail -1)" \
        'Generated by Harness(claude) and LLM(opus)'
    # ... and the block itself still took the NEW numbers (agy, PR #17).
    _ok 'replace_footer metrics updated' \
        "$(replace_footer "$newfmt" 9000 4 12 | grep -c '~9000 tokens')" '2'
    _ok 'replace_footer origin idempotent' \
        "$(replace_footer "$newfmt" 1500 2 6)" "$newfmt"
    _ok 'strip_footer origin'       "$(strip_footer "$newfmt")" "$body"

    # Pre-#16 wrapped card: no origin line at all. Detection is unchanged, and
    # --force upgrades it to the new shape with the unknown marker.
    noorigin=$(printf '%s\n\n---\n<details>\n<summary>AI Metrics</summary>\n\n<!-- ai-metrics -->\nold\n<!-- /ai-metrics -->\n\n</details>' "$body")
    _ok 'has_footer without origin' "$(has_footer "$noorigin" && echo yes || echo no)" 'yes'
    _ok 'has_footer with origin'    "$(has_footer "$newfmt"   && echo yes || echo no)" 'yes'
    _ok 'strip_footer without origin' "$(strip_footer "$noorigin")" "$body"
    _ok 'replace_footer upgrades no-origin' \
        "$(replace_footer "$noorigin" 1500 2 6)" "$tagged"

    # NF-3: the same sentence in ordinary prose is not the footer's line.
    prose=$(printf '%s\n\nSee Generated by Harness(x) and LLM(y) mid-body.\n\nTail.' "$body")
    prose_tagged=$(append_footer "$prose" 1500 2 6)
    _ok 'prose origin not replaced' \
        "$(replace_footer "$prose_tagged" 1500 2 6)" "$prose_tagged"
    _ok 'prose origin not stripped' "$(strip_footer "$prose_tagged")" "$prose"
