...
# `$2 // $o` re-emits the card's own line and falls back to the `none` form
# only when there was none, so a --force sweep never overwrites real
# provenance (full rationale: references/footer-detection.md).
replace_footer() {
