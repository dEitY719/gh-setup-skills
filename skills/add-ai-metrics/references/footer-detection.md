...
Since dEitY719/gh-issue-skills#43 the wrapped form carries one more visible
line, standing alone immediately after `</details>`:

```
Generated by Harness(<harness>) and LLM(<model>)
```

That byte format is owned upstream by `dEitY719/gh-issue-skills`'s
`lib/ai-metrics-footer.sh`. **This skill is a consumer and never redefines
it** — it detects the line, keeps it, and emits the unknown form when
backfilling. Three rules follow (issue #16):

- The line is part of the footer **unit**: `replace_footer` and `strip_footer`
  match it together with the block, so a `--force` re-run can never orphan or
  duplicate it (F-1 / F-4).
- The match is **optional** and **anchored immediately after `</details>`**, so
  a pre-#43 card still matches unchanged (D-2) and a `Generated by …` sentence
  sitting in ordinary prose elsewhere in the body is never swallowed (D-4).
- A backfill cannot know which harness or model wrote a card, so it never
  guesses: `Generated by Harness(none) and LLM(none)` when unknown, and the
  line already on the card verbatim when it is there (D-3). `has_footer` is
  deliberately **not** extended — the OPEN/CLOSE pair alone still decides, so
  the verdict is identical with or without the line (F-2).
...
The trailing `Generated by Harness(none) and LLM(none)` line follows it,
outside the wrapper so it stays visible when the block is collapsed — the
reason the upstream issue rejected moving it inside (gh-issue-skills#43 D-1).
Both values are `none`: this is a post-hoc backfill and the original author is
unknowable (F-3 / D-3).
...
A third optional group extends the match one line further:

```
(\n\nGenerated by Harness\([^\n]*\) and LLM\([^\n]*\))?
```

It is **capturing**, and the `/e` replacement re-emits what it captured —
falling back to `$ORIGIN` (the `none` form) only when the group did not
participate, i.e. on a pre-#43 card being upgraded in place. Regenerating the
line instead of preserving it would overwrite real provenance with `none` on
every `--force` sweep, and a backfill has no way to recompute it.
...
7. On a card carrying the provenance line, one `--force` run leaves exactly
   one `Generated by Harness(` occurrence, with the card's own harness/model
   values intact, and a second run is byte-identical to the first.
8. `has_footer` returns the same verdict with and without the provenance line.
9. A `Generated by Harness(x) and LLM(y)` sentence in ordinary prose mid-body
   survives both `replace_footer` and `strip_footer` untouched.
