...
- The trailing provenance line sits **outside** the wrapper so it stays
  visible when the block is collapsed. A backfill writes
  `Harness(none) and LLM(none)` — see `references/footer-detection.md`.
