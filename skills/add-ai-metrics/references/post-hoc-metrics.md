...
- `(?:\n\nGenerated by Harness\([^\n]*\) and LLM\([^\n]*\))?` optionally
  matches the provenance line that follows it (gh-issue-skills#43). Stripping
  it is what keeps the `--force` round-trip byte-exact; leaving it in would
  also feed its ~45 chars back into the token estimate — the exact feedback
  loop this strip exists to break. Anchored right after `</details>`, so the
  same sentence in prose is not a match (`references/footer-detection.md` D-4).
